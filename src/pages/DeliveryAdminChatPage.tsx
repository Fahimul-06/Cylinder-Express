  ...
  return <div className="min-h-screen bg-slate-50 pt-24 pb-6 px-3 sm:px-6">
    <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col h-[calc(100vh-8rem)]">
      <header className="p-4 border-b flex items-center gap-3 bg-blue-600 text-white">
        ...
        <div><h1 className="font-bold">Administration Head Live Support</h1><p className="text-xs text-blue-100 flex items-center gap-1"><Wifi className="w-3 h-3" /> Messages update live</p></div>
      </header>
      <main className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50">
        {messages.length === 0 && <div className="text-center text-sm text-gray-500 mt-16">Send a message to the Administration Head for delivery help, order issues, or account support.</div>}
        {messages.map(m => {
