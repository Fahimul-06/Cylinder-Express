...
type ChatMessage = { id: string; sender_id: string; sender_role: 'delivery' | 'admin'; message: string; created_at: string; sender_name?: string | null; sender_position?: string | null };
...
export default function AdminDeliveryChat() {
  ...
  return <div className="p-4 sm:p-6 lg:p-8"><div className="max-w-6xl mx-auto">
    <div className="mb-5"><h1 className="text-2xl font-bold text-gray-900">Delivery Live Messages</h1><p className="text-sm text-gray-500">Chat with HUB personnel in near real time.</p></div>
    ...
    <div className="bg-white border rounded-2xl overflow-hidden h-[72vh] grid md:grid-cols-[320px_1fr]">
      <aside className="border-r flex flex-col min-h-0">
        <div className="p-3 border-b"><div className="relative"><Search className="absolute left-3 top-3 w-4 h-4 text-gray-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search HUB man" className="w-full pl-9 pr-3 py-2.5 border rounded-xl text-sm" /></div></div>
        <div className="overflow-y-auto flex-1">
          ...
          {filtered.length === 0 && <p className="p-8 text-center text-sm text-gray-400">No HUB man found.</p>}
        </div>
      </aside>
      <section className="flex flex-col min-w-0 min-h-0">
        ...
        </> : <div className="flex-1 flex flex-col items-center justify-center text-gray-400"><MessageCircle className="w-12 h-12 mb-3" /><p>Select a HUB man to start chatting.</p></div>}
      </section>
    </div>
  </div></div>;
}
