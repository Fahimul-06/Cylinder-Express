...
app.post('/api/auth/signin', async (req, res) => {
  try {
    ...
    const audience = String(req.body.audience || 'customer').toLowerCase();
    ...
    const isAdminAccount = Boolean(profile?.is_admin) || role === 'admin' || role === 'sub_admin';
    const allowed = audience === 'admin'
      ? isAdminAccount
      : audience === 'delivery'
        ? role === 'delivery'
        : role === 'customer' && !isAdminAccount;
    if (!allowed) return res.status(403).json({ error: 'This account cannot sign in from this portal.' });
    const session = signUser(user);
    res.json({ session, user: session.user });
  } catch (error) { res.status(500).json({ error: error.message }); }
});
